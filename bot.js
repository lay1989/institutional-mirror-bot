...
async function manageOpenTrades() {
  ...
  for (const raw of open) {
    ...
    const before = { ...trade };
    // Falls back to trade open time for any trade created before this field existed
    const lastCheckedMs = raw.LastCheckedUTC ? new Date(raw.LastCheckedUTC).getTime() : trade.openTime.getTime();
    ...
    // Fetch recent 15m candles once, used for two different things:
    //  - candlesSinceEntry (full history since open) for structural swing detection
    //  - candlesSinceLastCheck (only new candles since the last run) for accurate
    //    TP/SL touch detection — this is the fix for the "wick in between checks"
    //    gap: instead of one point-in-time price, use the real high/low of
    //    whatever candles closed since we last looked, so a brief spike through
    //    a level that already reversed by this run still gets caught correctly.
    let candlesSinceEntry = [];
    let candlesSinceLastCheck = [];
    try {
      const m15 = await getKlines(symbol, '15m', 150); await sleep(300);
      candlesSinceEntry = m15.filter(c => c.openTime >= trade.openTime.getTime());
      candlesSinceLastCheck = m15.filter(c => c.openTime >= lastCheckedMs);
    } catch (err) {
      console.warn(`Candle fetch failed for ${raw.ID} (falling back to point-in-time price only): ${err.message}`);
    }

    const high = candlesSinceLastCheck.length
      ? Math.max(price, ...candlesSinceLastCheck.map(c => c.high))
      : price;
    const low = candlesSinceLastCheck.length
      ? Math.min(price, ...candlesSinceLastCheck.map(c => c.low))
      : price;

    const outcome = evaluateTradeExit({
      trade, high, low, close: price,
    ...
    if (outcome.closed) {
      ...
      const changed = { LastCheckedUTC: new Date().toISOString() };
      ...
      update = changed; // always write at least LastCheckedUTC so the next run's window is correct
    }
